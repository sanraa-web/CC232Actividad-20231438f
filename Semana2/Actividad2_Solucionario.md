...
| ------- | ------ | ---------------- | ---------------------- |
| demo_array_basico.cpp | array.length = 5 contenido: 10 20 30 40 50 antes de la asignacion, b[0] = -1 despues de b = a, b.length = 5 b: 10 20 30 40 50 | agregar datos, indexar y elminar | O(n) |
| demo_arraystack.cpp | size = 3  10 15 20 remove(1) = 15 | añadir elementos a la cola | O(n) |
| demo_fastarraystack.cpp | size = 7  0 1 2 99 3 4 5 | aumetar longitud del array | O(n) |
| demo_arraystack_explicado.cpp | Operacion 1-4 probando size y capacity | usar add(i,x) y remove(i) | O(n-i) |
| demo_rootisharraystack.cpp | size = 8  0 10 20 30 40 50 60 70 remove(3) = 30 | Comparar size luego de añadir y eliminar elementos. | O(n) |
